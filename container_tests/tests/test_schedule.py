...
class ScheduleTests(unittest.TestCase):
    def test_Schedule(self):
        ...
        self.assertEqual(sched.source_specs, ["/", "/var"])
        ...
        self.assertEqual(sched.policy, gcp)
    ...
    def test_Schedule_from_dict(self):
        sched_dict = {
            ...
            "autoindex": True,
            "calendarspec": "hourly",
            "policy": {'policy_name': 'test', 'policy_type': 'COUNT', 'keep_count': 4},
        }
        ...
        self.assertEqual(sched.autoindex, True)
        self.assertEqual(sched.calendarspec, "hourly")
        self.assertEqual(sched.policy, GcPolicy("test", GcPolicyType.COUNT, {"keep_count": 4}))
    ...
    def test_Schedule_from_dict_bad_keys(self):
        sched_dict = {
            ...
            "badpolicy": {'policy_name': 'test', 'policy_type': 'COUNT', 'keep_count': 4},
        }
        with self.assertRaises(KeyError) as cm:
            sched = Schedule.from_dict(sched_dict)

    def test_Schedule_from_file(self):
        sched = Schedule.from_file("/src/container_tests/test.conf")
        self.assertEqual(sched.name, "test")
        self.assertEqual(sched.source_specs, ["/", "/var"])
        self.assertEqual(sched.default_size_policy, "10%SIZE")
        self.assertEqual(sched.autoindex, True)
        self.assertEqual(sched.calendarspec, "hourly")
        self.assertEqual(sched.policy, GcPolicy("test", GcPolicyType.ALL, {}))

    def test_Schedule_from_file_bad_file(self):
        with self.assertRaises(snapm.SnapmSystemError) as cm:
            Schedule.from_file("/src/container_tests/nosuch.conf")

    def test_Schedule_from_file_bad_json(self):
        with self.assertRaises(JSONDecodeError) as cm:
            Schedule.from_file("/src/container_tests/badjson.conf")

    def test_Schedule_from_file_bad_keys(self):
        with self.assertRaises(KeyError) as cm:
            Schedule.from_file("/src/container_tests/badkeys.conf")
    ...
    "policy": {
        ...
        conf_file = os.path.join(conf_dir, "test.conf")
    ...
    def test_Schedule__str__(self):
        ...
        gcp = GcPolicy("test", GcPolicyType.AGE, {"keep_weeks": 2})
        sched = Schedule("test", ["/", "/var"], "10%SIZE", True, "weekly", gcp)
        self.assertEqual(str(sched), xstr)
    ...
    def test_Schedule__eq__(self):
        ...
        sched2 = Schedule.from_file("/src/container_tests/test.conf")
