...
class MountsTestsSimple(unittest.TestCase):
    ...
    def test__mount__umount(self):
        ...
        with tempfile.TemporaryDirectory(suffix="_test_mount") as tempdir:
            ...
            mounts._mount("none", mount_point, fstype="tmpfs", options="size=1m")

            self.assertTrue(ismount(mount_point))

            with open(path_join(mount_point, "testfile"), "w", encoding="utf8") as fw:
               fw.write(somestr)

            with open(path_join(mount_point, "testfile"), "r", encoding="utf8") as fr:
               self.assertEqual(fr.read(), somestr) 

            mounts._umount(mount_point)

            self.assertFalse(ismount(mount_point))

    def test__mount_bad_dev(self):
        with self.assertRaises(snapm.SnapmMountError):
            mounts._mount("/not/a/dev", "/tmp")
