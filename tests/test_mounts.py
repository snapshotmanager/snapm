...
class MountsTests(MountsTestsBase):
    ...
    def test_find_mounts(self):
        ...
        """
        # 1. No mounts
        self.assertEqual(self.mounts.find_mounts(snapm.Selection(name=self.snapset_name)), [])

        # 2. Mount and find
        mount_obj = self.mounts.mount(self.snapset)

        # Find by name
        found = self.mounts.find_mounts(snapm.Selection(name=self.snapset_name))
        self.assertEqual(found, [mount_obj])

        # Find all
        found = self.mounts.find_mounts(snapm.Selection())
        self.assertEqual(found, [mount_obj])

        # Find and miss
        found = self.mounts.find_mounts(snapm.Selection(name="no-such-snapset"))
