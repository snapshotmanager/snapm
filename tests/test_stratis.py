...
class StratisTests(unittest.TestCase):
    ...
    def test__get_pool_filesystem_bad_pool(self):
        ...
        with self.assertRaises(DbusClientUniqueResultError) as cm:
            stratis._get_pool_filesystem(managed_objects, "nosuchpool", "fs1")
    ...
    def test__get_pool_filesystem_bad_fs(self):
        ...
        with self.assertRaises(DbusClientUniqueResultError) as cm:
            stratis._get_pool_filesystem(managed_objects, _POOL_NAME, "nosuchfs1")

    def test_pool_free_space_bytes(self):
        proxy = get_object(TOP_OBJECT)
        managed_objects = ObjectManager.Methods.GetManagedObjects(proxy, {})

        (pool, _) = stratis._get_pool_filesystem(managed_objects, _POOL_NAME, "fs1")
    ...
    def test__origin_uuid_to_fs_name(self):
        ...
        (_, fs) = stratis._get_pool_filesystem(managed_objects, _POOL_NAME, "fs1-snapset_test_1721136677_-opt")
    ...
    def test_filter_stratis_snapshot_nonsnapshot(self):
        ...
        (_, fs) = stratis._get_pool_filesystem(managed_objects, _POOL_NAME, "fs1")
